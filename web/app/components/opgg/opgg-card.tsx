...
function getRankScore(
  ...
  const rankIndex = isMasterPlus
    ? 0
    : rank
    ? RANK_ORDER.indexOf(rank.toUpperCase())
    : 0;
  const lpValue = lp ?? 0;
...
function getHighestRankAccount(accounts: OpggAccount[]): OpggAccount | null {
  ...
  return accounts.reduce((highest, current) => {
    const highestScore = getRankScore(
      highest.tier,
      highest.rank,
      highest.league_points
    );
    const currentScore = getRankScore(
      current.tier,
      current.rank,
...
export default function OpggCard({ onClose, onMouseDown }: OpggCardProps) {
        ...
        if (allTabs.length > 0 && !activeTab) {
          ...
          if (activeTab.isAllTab) {
            ...
            const currentTab = allTabs.find(
              (t) => t.account?.id === activeTab.account?.id
            );
            if (currentTab) {
  ...
  // Handle LP click - navigate to the highest rank account's tab when on "All" tab
  const handleLpClick = useCallback(() => {
    if (isAllTab && highestRankAccount) {
      const targetTab = tabs.find(
        (t) => t.account?.id === highestRankAccount.id
      );
      if (targetTab) {
        setActiveTab(targetTab);
      }
    }
  }, [isAllTab, highestRankAccount, tabs]);
      ...
      >
        ...
        {floatingEmotes.map((emote) => (
          <FloatingEmote
            ...
            onComplete={handleEmoteComplete}
          />
        ))}
      </div>
